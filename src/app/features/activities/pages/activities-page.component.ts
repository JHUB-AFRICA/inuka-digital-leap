...
@Component({
    ...
    <section class="activities-page">
      <div class="container">
        <div class="activities-page__grid">
          @for (activity of filteredActivities(); track activity.title; let i = $index) {
            <a
              [routerLink]="['/activities', activity.slug]"
              class="activities-page__card"
              appScrollReveal="fade-up"
              [delay]="(i % 3) * 80"
            >
              <div class="activities-page__card-visual">
                <span
                  class="activities-page__card-icon"
                  [class]="activity.icon"
                  aria-hidden="true"
                ></span>
              </div>
              <div class="activities-page__card-body">
                <div class="activities-page__card-meta">
                  <span class="activities-page__card-category">{{ activity.category }}</span>
                  <span class="activities-page__card-duration">{{ activity.duration }}</span>
                </div>
  ...
  styles: [`
    ...
    .activities-page__card-category {
      font: var(--label-caps);
      font-size: 0.65rem;
      color: var(--color-electric-blue);
    ...
    .activities-page__card-duration {
      font-family: var(--font-body);
      font-size: 0.75rem;
      color: var(--color-on-surface-variant);
    }

    .activities-page__card-title {
      font-family: var(--font-heading);
      font-size: 1.05rem;
      font-weight: 700;
    ...
    .activities-page__card-description {
      font-family: var(--font-body);
      font-size: 0.85rem;
      line-height: 1.65;
      color: var(--color-on-surface-variant);
...
export class ActivitiesPageComponent {
  private readonly allActivities = [...ACTIVITIES];

  protected readonly categories = [
    ...new Set(this.allActivities.map((a) => a.category)),
  ].sort();
  ...
  protected readonly filteredActivities = computed(() => {
    ...
    return cat === 'All'
      ? this.allActivities
      : this.allActivities.filter((a) => a.category === cat);
  });
